...
function createDashboard(optionsMenu) {
    ...
    function installPhoneTagInstruction() {
        ...
        const instruction =
            ...
            'texting to an actual voice call, start that line with the ' +
            'exact tag [CALL] by itself. When that call naturally ends, ' +
            'end the final line of the call with the exact tag [HANGUP] ' +
            'by itself. Never use either tag for anything other than a ' +
            'real phone call actually starting or ending.';
    ...
    // Re-mirrors the real chat into the captions panel, showing only
    // whatever was said after the call connected.
    function renderCallCaptions() {
        ...
        callScreenCaptions.innerHTML = duringCall
            .map((message) => `
                <div class="call-caption ${message.from}">
                    ${(message.text || '')
                        .split('\n')
                        .map((line) => line.trim())
                        .filter((line) => line.length > 0)
        ...
        callScreenCaptions.scrollTop =
            callScreenCaptions.scrollHeight;

    }

    // Small "..." bubble shown while the character's line is still
    // generating — the call-screen equivalent of showTypingBubble().
    function showCallSpeakingBubble() {
        ...
        row.className = 'call-caption in call-caption-typing';
        ...
        callScreenCaptions.appendChild(row);
    ...
    async function connectCall() {
        ...
        try {
            ...
            // "/trigger" asks SillyTavern to generate the next turn
            // without posting a line of its own — the character
            // speaks first, the way answering a call would feel.
    ...
    function openIncomingCallScreen(characterName, entryIndexInChat) {
        ...
        // Purely cosmetic ring so this still feels like a call
        // arriving rather than a screen just appearing — the line
        // itself was already written by the model, there's nothing
        // to generate before showing it.
